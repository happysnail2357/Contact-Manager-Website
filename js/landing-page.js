...
$('#userinfo').on('show.bs.modal', function (event) 
    {
        ...
        $("#emoji-profile").click(function()
        {
            ...
            let emoji_name = Object.keys(emojis)[emoji_index];
        ...
        if(button.attr('id') != "create-btn"){
            ...
            url = "https://cardboardmc.com/LAMPAPI/GetContactInfo.php";
                
            const get_contact_info = {
                "userId": user_id,
                "contactId":button.attr("id")
            }
        ...
        //     });
            ...
            // modal.find('#email').text('New message to ' + recipient)
	...
	let html= `<tr id=t-row${jsonObject.ID}>
                ...
                <th scope="row"  > 
                    <div class="div d-flex justify-content-center mt-4">
                        <input class="form-check-input" type="checkbox" value="${jsonObject.ID}" style="height: 20px;width: 20px;">
                    </div>
                </th>
                    <td><p class=" text-center">${(jsonObject.Emoji in emojis)? jsonObject.Emoji: emojis["BigFrown"]}</p></td>
                    <td class="text-center"><div class="first-name">${jsonObject.FirstName? jsonObject.FirstName: "#"}</div><div class="last-name">${jsonObject.LastName? jsonObject.LastName: " "}</div></td>
                        ...
                        <td clas= "text-center">
                        <button type="button" class="btn btn-primary rounded m-2" data-bs-toggle="modal" data-bs-target="#userinfo" id = ${jsonObject.ID}>                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
